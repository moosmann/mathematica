...
Clear[pcg,scg,sp,rcg,rp];
pcg[num_,xy__List]:=
Module[{order=8 ,fpts=40,masspts=200,pts,n,newpts,spline,
xxpts,yypts,xexp,yexp,xfk,yfk,xpara,ypara,yfitdat,xfitdat,
initialcurve,area,length,l,L,usol,iuv,com},
Subscript[tf, num,0]=0;
pts={xy};
n=Length[pts];
pts=Append[pts,First[pts]];

newpts=Join[{pts[[-3]]},{pts[[-2]]},pts,{pts[[2]]},{pts[[3]]}];
spline=Splines`SplineFit[newpts, Splines`Cubic];

xxpts=Table[{i 2\[Pi]/fpts,spline[2+n i/fpts][[1]]},{i,0,fpts}];
yypts=Table[{i 2\[Pi]/fpts,spline[2+n i/fpts][[2]]},{i,0,fpts}];
...
l[i_] := length i/ masspts;L[u_?NumberQ] := NIntegrate[Sqrt[Evaluate[(\!\(
\*SubscriptBox[\(\[PartialD]\), \(p\)]\(initialcurve[p]\)\)).(\!\(
\*SubscriptBox[\(\[PartialD]\), \(p\)]\(initialcurve[p]\)\))]], {p, 0, u}];
usol[i_] := FindRoot[L[u] == l[i], {u, 0, 2 Pi}];
iuv =Table[usol[i], {i, 0, masspts-1}];
com=Total[initialcurve[u]/.iuv]/masspts;


Subscript[curve, num,1][u_]:=10/Sqrt[area/(2\[Pi])](initialcurve[u]-com);

Show[
ListPlot[10/Sqrt[area/(2\[Pi])]{xy},PlotStyle->{Red,PointSize[Large]},AspectRatio->Full],
ParametricPlot[11/Sqrt[area/(2\[Pi])]spline[2+n u/(2Pi)],{u,0,2Pi},AspectRatio->Full,PlotStyle->{LightGreen}],
ListPlot[Table[Subscript[curve, num,1][2Pi i/fpts],{i,0,fpts}],AspectRatio->Full],
ParametricPlot[Subscript[curve, num,1][u],{u,0,2\[Pi]},AspectRatio->Full]
]

];

scg[num_,width_,height_,kink_,touch_,asymmetrie_,diagonaldistortion_]:=
Module[{masspts=200,u,lo,initialcurve,aoi,
area,length,l,L,usol,iuv,com},
Subscript[tf, num,0]=0;
...
area=-1/2 NIntegrate[(initialcurve[u][[1]])(\!\(
...
uplot[num_,step_]:=Show[ParametricPlot[Subscript[curve, num,step][u],{u,0,2Pi}],ListPlot[Subscript[curve, num,step][u]/.Subscript[iuv, num]],Graphics[{Red,PointSize[Large],Point[Subscript[com, num]]}]]
...
dist[num_,step_,t_,i_,j_]:=Sqrt[((Subscript[x, i][t]/.Subscript[sol, num,step][[1,i+1]])-(Subscript[x, j][t]/.Subscript[sol, num,step][[1,j+1]]))^2+((Subscript[y, i][t]/.Subscript[sol, num,step][[1,i+Subscript[gpn, num,step]+2]])-(Subscript[y, j][t]/.Subscript[sol, num,step][[1,j+Subscript[gpn, num,step]+2]]))^2];
 ...
 iniequidist[num_,fitpts_] := 
  Module[{length,l,L,usol,time},
length = Subscript[initiallength, num];
l[i_] := length i/ fitpts;L[u_?NumberQ] := NIntegrate[Sqrt[Evaluate[(\!\(
\*SubscriptBox[\(\[PartialD]\), \(p\)]\(
\(\*SubscriptBox[\(curve\), \(num, 1\)]\)[p]\)\)).(\!\(
\*SubscriptBox[\(\[PartialD]\), \(p\)]\(
\(\*SubscriptBox[\(curve\), \(num, 1\)]\)[p]\)\))]], {p, 0, u}];
usol[i_] := FindRoot[L[u] == l[i], {u, 0, 2 Pi}];
Subscript[iuv, num] =Table[usol[i], {i, 0, fitpts-1}];
];
...
Clear[evo];
evo[num_,grd_:150,opts___]:=Module[{step,t=0,grid=grd},
...
Print[num,". curve, t_final - 100 = ",t-100,", steps = ",step];
]
...
Clear[lt,at,cut,cut2,lla,llafit,l];
lt[num_,t_]:=
Module[{i,l},
